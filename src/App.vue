  ...
  <nav class="fixed w-full bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900 z-50">
    <div class="container flex flex-wrap items-center justify-between mx-auto">
      <button @click="$router.push('/')" class="flex items-center">
        ...
        <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">Hayron竑</span>
      </button>
      ...
      <!-- mobile dark mode -->
      <button id="theme-toggle" type="button" @click="darkMode()" class="md:hidden text-gray-500 block pl-3 pr-4 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm">
      ...
      <div class="hidden w-full md:block md:w-auto" id="navbar-default">
        <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
          <li v-for="(element, key) in routes" :key="key" class="w-fit h-8 p-1 max-md:w-full flex lg:justify-center">
            <router-link :to="element.path" class="nav-link block pl-3 pr-4 text-gray-700 md:hover:bg-transparent md:border-0 md:hover:text-orange-400 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:text-white md:dark:hover:bg-transparent" aria-current="page">{{ element.name }}</router-link>
          </li>
        </ul>
      </div>
